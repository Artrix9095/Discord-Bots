...
class Battle(commands.Cog):
    def __init__(self, bot):
        ...
        with open(r"G:\Artrix-botz\PhysixsFun\cugo\weakness.json", 'r') as f:
            self.weak = json.load(f)
        ...
        if user == None:
            ...
            if off_id == def_id:
                ...
                if selected_mon or selected_mon1 == None:
                    ...
                    self.battle[user_names][off_id]['Selected']['HP'] = self.mon[selected_mon1]['HP'] * 2.5
                    self.battle[user_names][def_id]['Selected']['HP'] = self.mon[selected_mon]['HP'] * 2.5
                    ...
                    embed.set_author(name=f"Fight! - {ctx.author[:-5]} & {user.name[:-5]}", icon_url=self.bot.user.avatar_url)
                    ...
                    if self.broadcast == True:
                        ...
                        print("Error 4")
                        off_type = self.battle[user_names][off_id]['Selected']['Type']
                        def_type = self.battle[user_names][def_id]['Selected']['Type']
                        off_weak = self.weak[off_type]['Weakness']
                        def_weak = self.weak[def_type]['Weakness']
                        ...
                        if def_abil == "Regeneration" and def_health =< self.mon[selected_mon]['HP'] * 2.5:
                            def_health += def_health / 5
                        if off_abil == "Regeneration" and off_health =< self.mon[selected_mon1]['HP'] * 2.5:
                            off_health += off_health / 5
                        if atk > spatk:
                            dmg = round((atk * choices) / def1)
                            if def_abil == "Dual Swords":
                                if off_type == "SwordsMen":
                                    dmg *= 2
                                else:
                                    dmg *= 1.5
                            if def_type in off_weak:
                                dmg *= 2
                            self.def_atkz = True
                        else:
                        ...
                        if atk1 > spatk1:
                            ...
                            if off_abil == "Dual Swords":
                                if def_type == "SwordsMen":
                                    dmg1 *= 2
                                else:
                                    dmg1 *= 1.5
                            if off_type in def_weak:
                                dmg1 *= 2
                            self.off_atkz = True
                        else:
                            self.off_spatkz = True
                            dmg1 = round((spatk1 * choices) / spdef)
                            if off_type in def_weak:
                                dmg1 *= 2
